...
یعنی قصدمان تفسیر عدد 5- است؟ یا نه این منفی در یک عبارت ریاضی به کاربرده شده
...
و توکنهایی که به تنهایی قابل تفسیر نیستند
برای راحتی کار دو نوع فانکشن تعریف میکنیم برای هر کدام از انواع عملایت باینری و یونری
عملایت یونری چون به تنهایی قابل تفسیر هستند هیچ ورودی ای ندارند
...
در ای سازنده مپ های متد ها را به هر توکن متصل میکنیم و میگوییم هر توکن با چه متدی پردازش شود.
*/
...
برای مثال برنامه زیر را در نشر بگیرید
...
func (p *parser) parseExpression(priority int) Node {
	...
	/*
		...
		است و مقدار لفت یعنی 5 را به آن پاس میدهیم
		بعد از اجرای این متد سمت چپ معادله 5 است و سمت راست را با کال کردن
		parseExpression
...
سپس مقدار عددی آن را درون استراکت مروبوط به عدد صحیح قرار دهیم
*/
type Int struct {
	Token Token
	Value int
...
func (p *parser) parseBinaryOperation(left Node) Node {
	...
	/*
		...
		ابتدا اولویت توکن فعلی را محاسبه میکنیم سپس این الویت را به متد
		parseExpression
...
for i in ["A","B","C"] { }
مقدار شمارنده برابر با هر کاراکتر خواهد بود اما اگر دو شمارنده به کار ببریم اولی مثدار اندیس
...
func (p *parser) parseMap() Node {
	...
	for {
		...
		*/
		...
		/*
			اگر به براکت بسته رسیدیم از روی آن میپریم و کار پردازش مپ را تمام میکینم
		*/
